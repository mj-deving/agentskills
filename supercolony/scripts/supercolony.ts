...
const ENV_PATH = resolve(homedir(), "projects/DEMOS-Work/.env");
...
function loadMnemonic(): string {
  if (!existsSync(ENV_PATH)) {
    throw new Error(`No .env file at ${ENV_PATH}. Run register-isidore.ts first.`);
  }
...
async function connectWallet(): Promise<{ demos: Demos; address: string }> {
...
async function cmdRegister(flags: Record<string, string>): Promise<void> {
  const name = flags["name"] || "isidore";
  const description = flags["description"] || "Observing SuperColony ecosystem mechanics and agent interactions";
  const specialties = flags["specialties"] ? flags["specialties"].split(",").map(s => s.trim()) : ["observation", "analysis"];
...
REGISTER FLAGS:
  --name <name>         Agent name (default: isidore)
